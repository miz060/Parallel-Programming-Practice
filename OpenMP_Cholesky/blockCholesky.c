 ...
 * File name: blockCholesky.c
 * File description: serial (in blockCholesky) block cholesky LU factorization
...
void blockCholesky(double *L, double*A, int N, int blk, int itr){
  ...
  for(itr=0; itr<N/blk; itr++){
    ...
    //printf("itr is %d\n", itr);
    /*printf("L21 before this itr is \n");
      for(i=0; i<(N-blk*(itr+1)); i++){
      for(j=0; j<blk; j++){
      printf("%15.8f ", L21[IDX(i,j,N)]);
      }
      printf("\n");
      }*/

    // calculate top triangular matrix L11. write L11 to L
    ...
    for(j=0; j<blk; j++){
      ...
      for(i=0; i<N-blk*(itr+1); i++){
        ...
        //printf("L21(%d,%d) result is %f\n",i,j,L21[IDX(i,j,N)]);
      }
    }
    /*printf("L21 after this itr is \n");
      for(i=0; i<(N-blk*(itr+1)); i++){
      for(j=0; j<blk; j++){
      printf("%15.8f ", L21[IDX(i,j,blk)]);
      }
      printf("\n");
      }*/
    ...
    // write local copies to the global L
...
int main(int argc, char* argv[]) {
  ...
  printf(" %dX%d block matrix with blk=%d\n", n,n, blk);
  ...
  // copy the element of A into A_copy
  for(i=0; i<N; i++){
    for(j=0; j<N; j++){
  ...
  cs160validate(A_copy,L,N,THRESH);
  free(A);
  free(A_copy);
  free(L);
  return badcount;
